...
# # @login_required(login_url='login')
# @allowed_user(allowed_roles=['dealer'])
# def dealer_dash(request):
#     return render(request,'dealer_dash.html')
...
def makeShipment(request):
    ...
    if "shipment_info" in request.POST:
        ...
        # packageLength = request.POST['packageLength']
        # packageWidth = request.POST['packageWidth']
        # packageHeight = request.POST['packageHeight']
        packageSize = request.POST['packagesize']
        packageWeight = request.POST['packageWeight']
        context['locatedealer'] = {}
        print(packageSize)
        # print(packageWeight,fragile,senderAddress_zip,senderAddress_city,senderAddress_country)
        # print(recipientAddress_city,recipientAddress_zip,recipientAddress_country)
        
        #storing the values so that when reload of page the variable will not reset
        # request.session['packageLength'] = packageLength
        # request.session['packageWidth'] = packageWidth
        # request.session['packageHeight'] = packageHeight
...
def paymentgateway(request):
    ...
    # price=float(request.session.get('amount'))
    # print('orderid',request.session.get('orderid'))
    # oid= str(request.session.get('orderid'))
    # client = razorpay.Client(auth=("rzp_test_rOgiQUix1apLnN", "Kvhw6yh0fS0JQKwgSE4ZxbKy"))
    # data = { "amount": price*100, "currency": "INR", "receipt": "receiptid_"+oid }
    # payment = client.order.create(data=data)
    # print(payment)
